...
class Graphics:
	...
	def createAlphabet(self):
		...
		letters = self.characterSet[:13], self.characterSet[13:] #('ABCDEFGHIJKLM', 'NOPQRSTUVWXYZ')
		fst = { letter : self.createLetter(letter, (10+N*15, self.size.height-40)) for N, letter in enumerate(letters[0]) }
		snd = { letter : self.createLetter(letter, (10+N*15, self.size.height-20)) for N, letter in enumerate(letters[1]) }
		fst.update(snd)
		return fst
